...
add_action( 'init', __NAMESPACE__ . '\wporg_icons_filepath_shim', PHP_INT_MAX );
...
function wporg_icons_filepath_shim() {
      ...
      try {
              ...
              // NB: do NOT use $registry->get_registered_icons() - it eagerly calls
      ...
      foreach ( $icons as $name => $icon ) {
              ...
              // without the new snake_case one. Once Gutenberg's code is updated to write
              // `file_path` (core #64847), `file_path` is already set (or `filePath` is gone),
              // this is false for every icon, and the shim writes nothing — it's inert.
