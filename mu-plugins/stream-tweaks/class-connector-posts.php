 ...
 * Connector for Posts
 *
 * This is a modified version of the Posts connector that ships with version 3.8.2 of the Stream plugin.
 *
 * Differences:
 * - The `register_frontend` prop override has been removed so that it defaults back to `true`. This allows the
 *   connector to log post changes made in an API context (i.e. in the block editor). This change has also been made in
 *   the Stream plugin itself but has not been released as of version 3.8.2.
 * - Adds a filter hook (with a wporg prefix) for modifying the list of action labels.
 * - Adds a filter hook (with a wporg prefix) for modifying the summary content of the generated log entry.
 */
 ...
 */
class Connector_Posts extends \WP_Stream\Connector {
	/**
	 * Connector slug
	 *
	 * @var string
	...
	public function get_action_labels() {
		return apply_filters( 'wporg_stream_' . $this->name . '_action_labels', array(
	...
	public function callback_transition_post_status( $new, $old, $post ) {
		...
		/**
		 * Filter: Modify the log summary.
		 *
		 * @param string   $summary The summary string to be modified.
		 * @param string   $new     The new status of the post.
		 * @param string   $old     The old status of the post.
		 * @param \WP_Post $post    The post object.
		 */
		$summary = apply_filters( 'wporg_stream_' . $this->name . '_log_summary', $summary, $new, $old, $post );
